...
@section('content')
                ...
                <div class="bg-gray-50 rounded-lg p-4">
                    ...
                    <div class="mt-3">
                        ...
                        <div class="w-full bg-gray-200 rounded-full h-2">
                            <div class="bg-blue-600 h-2 rounded-full transition-all duration-300" id="detalleBarraProgreso" style="width: 0%"></div>
                        </div>
                    </div>
                </div>
    ...
    <script>
        ...
        function renderizarCalendario() {
            ...
            for (let i = 0; i < 7; i++) {
                ...
                if (cronograma) {
                    ...
                    const atendidos = 15 - cronograma.cantDispo;
                    const porcentaje = Math.round((atendidos / 15) * 100);
                                ...
                                <div class="space-y-3 flex-grow">
                                    <div class="turnos-info rounded-lg px-3 py-2">
                                        <div class="flex justify-between items-center mb-1">
                                            <span class="text-xs font-medium text-gray-700">Turnos</span>
                                            <span class="material-icons text-sm text-blue-600">${iconoEstado}</span>
                                        </div>
                                        <div class="flex items-baseline gap-1">
                                            <span class="text-2xl font-bold text-blue-700">${cronograma.cantDispo}</span>
                                            <span class="text-sm text-gray-600">disponibles</span>
                                        </div>
                                        <div class="text-xs text-gray-600 mt-1">
                                            ${atendidos} atendidos de 15
                                        </div>
                                    </div>
        ...
        function verDetalleCronograma(fecha, cronograma) {
            ...
            const atendidos = 15 - disponibles;
            const porcentajeOcupado = Math.round((atendidos / 15) * 100);
            ...
            document.getElementById('detalleTotales').textContent = '15';
            ...
            if (cronograma.estado === 'activo') {
                if (disponibles > 0) {
                    infoTexto = `Este cronograma está activo y tiene ${disponibles} turnos disponibles. Se han atendido ${atendidos} turnos hasta el momento.`;
                } else {
                    infoTexto = `Este cronograma está activo pero ya no tiene turnos disponibles. Todos los 15 turnos han sido reservados.`;
                }
            } else if (cronograma.estado === 'inactivoFut') {
                infoTexto = `Este cronograma está programado para una fecha futura. Tendrá 15 turnos disponibles cuando se active.`;
            } else {
                if (atendidos > 0) {
                    infoTexto = `Este cronograma ya finalizó. Se atendieron ${atendidos} de 15 turnos programados (${porcentajeOcupado}%).`;
                } else {
                    infoTexto = `Este cronograma ya finalizó. No se atendió ningún turno.`;
                }
            }
